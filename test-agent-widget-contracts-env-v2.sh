...
echo "=========================================="
echo " Testing agent_widget_contracts from app env"
echo "=========================================="
echo ""

if [ ! -f "$ENV_FILE" ]; then
  echo "ERROR: No existe .env.local en esta carpeta:"
  pwd
  exit 1
fi

echo "Variables Supabase encontradas en .env.local:"
...
SUPABASE_ANON_KEY="$(
  grep -E '^(VITE_SUPABASE_ANON_KEY|SUPABASE_ANON_KEY|PUBLIC_SUPABASE_ANON_KEY)=' "$ENV_FILE" \
    | tail -1 \
    | cut -d= -f2- \
    | sed 's/^["'\'']//; s/["'\'']$//'
)"
...
if [ -z "${SUPABASE_ANON_KEY:-}" ]; then
  echo "ERROR: No encontré anon key Supabase."
  echo "Busqué:"
...
echo "Probando REST con anon key del frontend..."
echo ""
...
if [ "$HTTP_STATUS" = "200" ]; then
  if grep -q "manual_test" "$HTTP_RESPONSE_FILE"; then
    ...
    echo "Entonces el problema está en el frontend/runtime, no en Supabase."
    ...
    echo "Casi seguro estás insertando la fila en otro proyecto Supabase diferente al .env.local."
  else
