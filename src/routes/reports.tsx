...
function ReportsPage() {
  ...
  const fetchReports = useCallback(async () => {
    ...
    const [{ data: leads }, { data: deals }, { data: invoices }, { data: tasks }] =
      await Promise.all([
    ...
    setMetrics({
      ...
      conversionRate:
        dealsArr.length > 0 ? Math.round((wonDeals.length / dealsArr.length) * 100) : 0,
    });
    ...
    dealsArr.forEach((d: any) => {
      stageMap[d.stage] = (stageMap[d.stage] || 0) + Number(d.value);
    });
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Card className="border-0 shadow-sm">
          <CardHeader className="pb-2">
            <CardTitle className="text-sm">Deal Value by Stage</CardTitle>
          </CardHeader>
          <CardContent>
