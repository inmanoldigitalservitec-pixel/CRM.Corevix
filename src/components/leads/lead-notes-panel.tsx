...
export function LeadNotesPanel({ leadId, canEdit }: { leadId: string; canEdit: boolean }) {
  ...
  const [selectedNote, setSelectedNote] = useState<LeadNoteRow | null>(null);
  ...
  const loadNotes = useCallback(async () => {
    ...
    const { data, error } = await (supabase as any)
      .from("lead_notes")
      .select(
        "id,company_id,lead_id,author_profile_id,kind,content,created_at,updated_at,archived_at,author:profiles!lead_notes_author_profile_id_fkey(id,full_name,email)",
      )
    ...
    if (error) {
      toast.error(error.message || "No se pudieron cargar las notas del prospecto.");
      setLoading(false);
      return;
    }
  ...
  async function handleArchiveNote(note: LeadNoteRow) {
    if (!profile?.company_id) return;
    setSaving(true);
    const { error } = await (supabase as any)
      .from("lead_notes")
      ...
      >
        ...
        >
          <Textarea
            ...
            placeholder="Ej: Se llamó al prospecto, pidió propuesta para el viernes..."
