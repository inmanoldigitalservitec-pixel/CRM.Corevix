...
function fileKindLabel(file: LeadDriveFileRow) {
...
function uploadLeadFileWithProgress(args: {
  leadId: string;
  file: File;
  token: string;
  ...
  return new Promise((resolve, reject) => {
    ...
    body.append("lead_id", args.leadId);
...
export function LeadFilesPanel({
  ...
  const [files, setFiles] = useState<LeadDriveFileRow[]>([]);
  ...
  const loadFiles = useCallback(async () => {
    ...
    if (error) {
      toast.error(error.message || "No se pudieron cargar los archivos del prospecto.");
      setLoading(false);
      return;
    }
  ...
  async function uploadFile(file: File) {
    ...
    setUploadProgress(0);
    try {
      const payload = await uploadLeadFileWithProgress({
        leadId,
        file,
        token,
      ...
      await loadFiles();
      toast.success("Archivo subido al prospecto.");
    } catch (error) {
  ...
  async function deleteDriveFile(file: LeadDriveFileRow) {
    ...
    await loadFiles();
    toast.success("Archivo eliminado del prospecto.");
  }
    ...
    <div className="mx-auto max-w-4xl space-y-4">
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-500">
              Documentos, imágenes y recursos externos vinculados a {leadName}.
            </p>
          </div>
      ...
      {loading ? (
        ...
        <EmptyState
          ...
          title="Sin archivos del prospecto"
