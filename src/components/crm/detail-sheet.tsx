        ...
        {hasHeaderContent ? (
          <SheetHeader className="relative border-b px-5 py-4 text-left">
            ...
            <div className="relative flex items-start justify-between gap-4">
              ...
              <button type="button" onClick={handleClose} className="grid h-9 w-9 place-items-center rounded-[12px] border bg-background/90 text-muted-foreground shadow-[0_10px_22px_rgba(15,23,42,.06)] transition hover:-translate-y-[1px] hover:bg-background" aria-label={t("common.close")} title={t("common.close")}>
                <X className="h-4 w-4" />
              </button>
            </div>
              ...
              <div data-demo={fieldGroupDataDemo} className="grid grid-cols-2 gap-4">
                {fields.map((field) => (
                  <div key={field.label} className={field.type === "tags" ? "col-span-2" : ""}>
                    ...
                    {field.type === "badge" && field.value ? <div className="mt-1"><StatusBadge status={String(field.value)} /></div> : field.type === "currency" ? <p className="mt-0.5 text-sm font-semibold">${Number(field.value || 0).toLocaleString()}</p> : field.type === "tags" && field.value ? <div className="mt-1 flex flex-wrap gap-1">{String(field.value).split(",").map((tag) => <span key={tag} className="rounded-full bg-muted px-2 py-0.5 text-[10px] text-muted-foreground">{tag.trim()}</span>)}</div> : <p className="mt-0.5 text-sm font-medium">{field.value || t("common.none")}</p>}
                  </div>
                ))}
              </div>
            ) : null}
