...
function customerLabel(row: CRMRow) {
  return firstText(row, ["company_name", "name", "company", "full_name", "contact_person", "display_name", "business_name", "title"], "Sin nombre");
}
...
function catalogLabel(row: CRMRow) {
  return firstText(row, ["name", "title", "service_name", "product_name", "package_name", "membership_name"], "Artículo sin nombre");
}
...
function catalogPrice(row: CRMRow) {
  return firstNumber(row, ["base_price", "price", "unit_price", "sale_price", "amount", "monthly_price", "total", "value"], 0);
}
...
export function InvoiceBuilderTest() {
  ...
  const [dueDate, setDueDate] = useState(new Date(Date.now() + 1000 * 60 * 60 * 24 * 15).toISOString().slice(0, 10));
  ...
  const [notes, setNotes] = useState("Gracias por confiar en nuestro equipo. Esta factura puede ser pagada mediante transferencia bancaria o enlace de pago.");
  const [terms, setTerms] = useState("Pago requerido antes de la fecha de vencimiento. Los servicios inician luego de confirmar el pago inicial.");

  const [items, setItems] = useState<InvoiceItem[]>([
    { id: uid(), name: "Configuración CRM", description: "Diseño, personalización y configuración inicial del CRM.", quantity: 1, price: 85000 },
    { id: uid(), name: "Automatización comercial", description: "Flujos automáticos para leads, clientes y seguimiento.", quantity: 1, price: 25000 },
  ]);
  ...
  useEffect(() => {
    async function loadCRMData() {
      ...
      const [clientsRes, leadsRes, productsRes, servicesRes, membershipsRes, packagesRes] = await Promise.all([
      ...
      if (Array.isArray(clientsRes.data)) {
        for (const row of clientsRes.data) {
          nextCustomers.push({ id: row.id, type: "client", name: customerLabel(row), email: customerEmail(row), phone: customerPhone(row), address: customerAddress(row), raw: row });
        }
      }
      if (Array.isArray(leadsRes.data)) {
        for (const row of leadsRes.data) {
          nextCustomers.push({ id: row.id, type: "prospect", name: customerLabel(row), email: customerEmail(row), phone: customerPhone(row), address: customerAddress(row), raw: row });
        }
      }

      const buildCatalog = (rows: CRMRow[] | null | undefined, type: CatalogOption["type"]) =>
        Array.isArray(rows)
          ? rows.map((row) => ({ id: row.id, type, label: catalogLabel(row), description: catalogDescription(row), price: catalogPrice(row), raw: row }))
          : [];
      ...
      if (membershipsRes.error) console.warn("No se pudieron cargar membresías", membershipsRes.error);
  ...
  const publicUrl = savedInvoice?.public_token ? `${window.location.origin}/invoice/public/${savedInvoice.public_token}` : null;
  ...
  function applyCatalog(itemId: string, catalogKey: string) {
    ...
    updateItem(itemId, { catalogId: selected.id, catalogType: selected.type, name: selected.label, description: selected.description, price: selected.price });
  }

  function addItem() {
    setItems((current) => [...current, { id: uid(), name: "", description: "", quantity: 1, price: 0 }]);
  }
  ...
  async function saveInvoice(nextStatus = status) {
    ...
    const validRows = items
      .map((item) => ({ ...item, name: String(item.name || "").trim(), description: String(item.description || item.name || "").trim(), quantity: Number(item.quantity || 0) || 0, price: Number(item.price || 0) || 0 }))
    ...
    try {
      ...
      const primaryProduct = validRows.find((item) => item.catalogType === "product" && item.catalogId);
      const invoiceData = {
        source: "invoice-builder-test",
        customer: { type: customerType, id: customerId, name: clientName, email: clientEmail, phone: clientPhone, address: clientAddress },
        clientName,
        clientEmail,
        clientPhone,
        clientAddress,
        ...
        taxRate,
        terms,
        items: validRows.map((item) => ({ catalogId: item.catalogId || null, catalogType: item.catalogType || null, name: item.name, description: item.description, quantity: item.quantity, unit_price: item.price, total: item.quantity * item.price })),
      };
      ...
      const itemRows = validRows.map((item) => ({ invoice_id: invoiceId, description: item.description || item.name, quantity: item.quantity, unit_price: item.price, total: item.quantity * item.price }));
      if (itemRows.length) {
    ...
    <div className="min-h-[calc(100vh-88px)] bg-[#f5f7fb] text-slate-950 print:bg-white">
      ...
      <div className="no-print sticky top-0 z-30 flex h-16 items-center justify-between border-b bg-white/95 px-5 backdrop-blur">
        <div>
          ...
          <p className="text-xs text-slate-500">Selecciona cliente/prospecto y catálogo. Lo demás va a avanzado.</p>
        </div>
        <div className="flex items-center gap-2">
          <button type="button" onClick={printInvoice} className="inline-flex items-center gap-2 rounded-full border px-4 py-2 text-sm font-semibold hover:bg-slate-50"><Printer className="h-4 w-4" />PDF</button>
          <button type="button" disabled={saving} onClick={() => void saveInvoice("Sent")} className="inline-flex items-center gap-2 rounded-full bg-blue-600 px-4 py-2 text-sm font-semibold text-white hover:bg-blue-700 disabled:opacity-60"><Send className="h-4 w-4" />{saving ? "Guardando…" : "Enviar"}</button>
        </div>
      </div>
        ...
        <aside className="no-print flex min-h-0 flex-col overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
          <div className="flex items-center justify-between gap-4 border-b p-4">
            <div className="flex items-center gap-3">
              <div className="grid h-10 w-10 place-items-center rounded-2xl bg-blue-50 text-blue-600"><Sparkles className="h-5 w-5" /></div>
              <div>
                <h2 className="text-lg font-black">Nueva factura asistida</h2>
                <p className="text-xs text-slate-500">{loadingCRMData ? "Cargando CRM…" : savedInvoice ? `Guardada: ${savedInvoice.number || invoiceNumber}` : "Solo 2 pasos: cliente y catálogo."}</p>
              </div>
            </div>
            <button type="button" onClick={() => setShowAdvanced(true)} className="inline-flex items-center gap-2 rounded-full border px-4 py-2 text-sm font-bold hover:bg-slate-50"><Settings2 className="h-4 w-4" />Avanzado</button>
          </div>
          ...
          <div className="grid gap-4 border-b bg-slate-50 p-4 lg:grid-cols-[1fr_190px]">
            <label className="grid gap-2 text-xs font-black uppercase tracking-[0.12em] text-slate-400">
              Cliente o prospecto
              <select value={selectedCustomerKey} onChange={(e) => applyCustomer(e.target.value)} className="h-14 rounded-2xl border bg-white px-4 text-base font-bold normal-case tracking-normal outline-none focus:border-blue-400">
                <option value="">Seleccionar cliente o prospecto…</option>
                <optgroup label="Clientes">
                  {customers.filter((c) => c.type === "client").map((customer) => <option key={`client:${customer.id}`} value={`client:${customer.id}`}>{customer.name}</option>)}
                </optgroup>
                <optgroup label="Prospectos">
                  {customers.filter((c) => c.type === "prospect").map((customer) => <option key={`prospect:${customer.id}`} value={`prospect:${customer.id}`}>{customer.name}</option>)}
                </optgroup>
              </select>
            </label>
          ...
          <div className="min-h-0 flex-1 overflow-hidden p-4">
            <div className="mb-3 flex items-center justify-between gap-3">
              <div><h3 className="font-black text-slate-950">Catálogo</h3><p className="text-xs text-slate-500">Productos, servicios, membresías y paquetes.</p></div>
              <button type="button" onClick={addItem} className="inline-flex items-center gap-1 rounded-full bg-blue-600 px-4 py-2 text-xs font-bold text-white hover:bg-blue-700"><Plus className="h-3.5 w-3.5" />Agregar</button>
            </div>

            <div className="flex max-h-full flex-col overflow-hidden rounded-2xl border border-slate-200">
              <div className="grid grid-cols-[34px_1fr_86px_138px_38px] gap-2 bg-slate-50 px-3 py-2 text-[11px] font-black uppercase tracking-[0.12em] text-slate-400"><span>#</span><span>Producto / Servicio / Membresía / Paquete</span><span>Cant.</span><span>Total</span><span /></div>
              <div className="min-h-0 flex-1 divide-y divide-slate-100 overflow-auto">
                {items.map((item, index) => (
                  <div key={item.id} className="grid grid-cols-[34px_1fr_86px_138px_38px] items-center gap-2 px-3 py-3">
                    <span className="grid h-8 w-8 place-items-center rounded-full bg-slate-100 text-xs font-black text-slate-500">{index + 1}</span>
                    <select value={item.catalogType && item.catalogId ? `${item.catalogType}:${item.catalogId}` : ""} onChange={(e) => applyCatalog(item.id, e.target.value)} className="h-12 min-w-0 rounded-xl border px-3 text-sm font-bold outline-none focus:border-blue-400">
                      <option value="">Seleccionar del catálogo…</option>
                      {catalog.map((entry) => <option key={`${entry.type}:${entry.id}`} value={`${entry.type}:${entry.id}`}>{catalogTypeLabel(entry.type)} · {entry.label} · {money(entry.price)}</option>)}
                    </select>
                    <input value={item.quantity} onChange={(e) => updateItem(item.id, { quantity: Number(e.target.value) || 0 })} type="number" className="h-12 rounded-xl border px-3 text-sm outline-none focus:border-blue-400" />
                    <div className="rounded-xl bg-slate-50 px-3 py-3 text-sm font-black text-slate-950">{money(item.quantity * item.price)}</div>
                    {items.length > 1 ? <button type="button" onClick={() => removeItem(item.id)} className="grid h-10 w-10 place-items-center rounded-xl text-red-500 hover:bg-red-50" title="Eliminar"><Trash2 className="h-4 w-4" /></button> : <span />}
                  </div>
                ))}
              </div>
            </div>
