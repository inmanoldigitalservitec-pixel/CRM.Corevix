...
import { Search, ChevronDown, SlidersHorizontal, MessageCircle } from "lucide-react";
...
export function MessengerReadonlyList({
  conversations,
  ...
  return (
    <aside
      data-demo="messenger-conversation-list"
      className={cn(
    ...
    >
      <div className="px-3.5 py-3 border-b border-black/10 bg-[#f0f2f5] space-y-2.5">
        ...
        {companyId ? (
          <div className="rounded-2xl border border-slate-200 bg-white px-3 py-2 text-[11px] text-slate-600 shadow-sm max-[820px]:hidden space-y-0.5">
            <div>Compañía actual: {companyId}</div>
            <div>Conversaciones Messenger cargadas: {conversations.length}</div>
          </div>
        ) : null}
      ...
      <div className="flex-1 min-h-0 overflow-y-auto p-0 bg-white">
        {loading ? (
          ...
          <div className="space-y-0">
            {filtered.map((c) => {
              const name = c.sender_name || "Usuario de Messenger";
                  ...
                  <div className="flex items-start gap-2.5">
                    ...
                    <div className="min-w-0 flex-1 max-[820px]:hidden">
                      ...
                      <div className="mt-1.5 flex flex-wrap gap-1">
                        <span className="inline-flex items-center h-4.5 px-1.5 rounded-full border text-[9.5px] font-medium whitespace-nowrap bg-blue-50 text-blue-700 border-blue-200">
                          Messenger
                        </span>
                        {statusLabel ? (
