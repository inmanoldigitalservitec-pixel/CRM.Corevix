...
export function InstagramReadonlyList({
  ...
  const filtered = useMemo(() => {
    ...
    return conversations.filter((c) => {
      if (!q) return true;
      const hay = [c.sender_name, c.external_user_id, c.last_message_text, c.status].filter(Boolean).join(" ").toLowerCase();
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 min-h-0 overflow-y-auto p-0 bg-white">
        {loading ? (
          <div className="p-3 text-sm text-muted-foreground max-[820px]:hidden">Cargando conversaciones de Instagram…</div>
        ) : error ? (
          <div className="p-3 text-sm text-destructive max-[820px]:hidden">No se pudieron cargar las conversaciones de Instagram.</div>
        ) : filtered.length === 0 ? (
          <div className="p-4 text-sm text-muted-foreground max-[820px]:hidden space-y-1.5">
            <p className="font-medium text-slate-700">Todavía no hay conversaciones de Instagram.</p>
          </div>
        ) : (
                  ...
                  <div className="flex items-start gap-2.5">
                    ...
                    <div className="min-w-0 flex-1 max-[820px]:hidden">
                      <div className="flex items-start justify-between gap-2 min-w-0">
                        ...
                        <div className="shrink-0 text-[11px] text-slate-400 pt-0.5">{formatMetaTime(c.last_message_at || c.created_at)}</div>
                      </div>
                      <div className="mt-1 truncate text-[12px] text-slate-500 leading-tight">{c.last_message_text || "Sin mensajes todavía"}</div>
