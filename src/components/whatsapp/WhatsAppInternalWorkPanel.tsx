...
type Mode = "task" | "note";
type TaskItem = { id: string; title: string; description: string | null; due_date: string | null; priority: string };
...
function contactName() {
  const panel = slot()?.closest("aside") || taskButton()?.closest("aside") || noteButton()?.closest("aside");
...
export function WhatsAppInternalWorkPanel() {
  ...
  useEffect(() => {
    ...
    function onClick(event: MouseEvent) {
      ...
      if (task) {
        const id = new URL(task.href, window.location.origin).searchParams.get("conversationId") || "";
        openPanel("task", id);
      } else {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        if (!cancelled && !tasksResult.error) setTasksByChat((prev) => ({ ...prev, [k]: (tasksResult.data || []).map(taskFromRow) }));
      } catch {
      ...
      try {
        const notesResult = await db
          ...
          .order("created_at", { ascending: false })
          .limit(5);
        if (!cancelled && !notesResult.error) setNotesByChat((prev) => ({ ...prev, [k]: (notesResult.data || []).map(noteFromRow) }));
      } catch {
  ...
  async function saveTask() {
    ...
    const relationalPayload = { ...basePayload, whatsapp_conversation_id: conversationId || null, whatsapp_channel: "whatsapp-web" };

    setSaving(true);
    try {
      const db = supabase as any;
      let result = await db.from("tasks").insert(relationalPayload).select("id,title,description,due_date,priority").single();
      if (result.error) result = await db.from("tasks").insert(basePayload).select("id,title,description,due_date,priority").single();
      if (result.error) throw result.error;
      const item = taskFromRow(result.data || basePayload);
      setTasksByChat((prev) => ({ ...prev, [chatKey]: [item, ...(prev[chatKey] || [])].slice(0, 5) }));
      setForm(emptyTask());
  ...
  async function saveNote() {
    ...
    try {
      ...
      const result = await db
        .from("whatsapp_internal_notes")
        .insert({ company_id: profile.company_id, whatsapp_conversation_id: conversationId || null, whatsapp_channel: "whatsapp-web", body, created_by: user?.id || profile.user_id || null })
        .select("id,body,created_at")
        .single();
      ...
      setNotesByChat((prev) => ({ ...prev, [chatKey]: [item!, ...(prev[chatKey] || [])].slice(0, 5) }));
      setNote("");
  ...
  return createPortal(
    <section className="rounded-2xl border border-[#bcebd0] bg-[#f0fff6] p-3 shadow-sm">
      <div className="mb-2 flex items-center justify-between gap-2">
        ...
        <button type="button" onClick={() => setOpen(false)} className="grid h-8 w-8 place-items-center rounded-xl border border-[#cfe2d9] bg-white text-[#52645d] hover:bg-[#f7fbf9]" title="Cerrar" disabled={saving}>
          <X className="h-4 w-4" />
        </button>
      </div>

      <p className="mb-2 rounded-xl bg-white/80 px-3 py-2 text-[11px] leading-4 text-[#60736b]">Esto es interno y pertenece solo a este chat. El cliente no lo verá.</p>
      {loading ? <p className="mb-2 text-[11px] font-bold text-[#7b8d86]">Cargando historial interno...</p> : null}

      {mode === "task" && tasks.length ? (
        <div className="mb-2 space-y-1.5">
          {tasks.slice(0, 3).map((task) => (
            <div key={task.id} className="rounded-xl border border-[#cfe2d9] bg-white px-3 py-2">
              <div className="flex items-center justify-between gap-2"><p className="truncate text-xs font-bold text-[#12231d]">{task.title}</p><CheckCircle2 className="h-4 w-4 shrink-0 text-[#00a884]" /></div>
              <p className="mt-0.5 truncate text-[11px] text-[#7b8d86]">To Do · {task.priority} · {task.due_date || "Sin fecha"}</p>
            </div>
          ))}
        </div>
      ) : null}
        ...
        <div className="mb-2 space-y-1.5">
          {notes.slice(0, 3).map((item) => (
            <div key={item.id} className="rounded-xl border border-[#cfe2d9] bg-white px-3 py-2">
              <div className="flex items-start justify-between gap-2"><p className="line-clamp-2 text-xs font-bold text-[#12231d]">{item.body}</p><CheckCircle2 className="h-4 w-4 shrink-0 text-[#00a884]" /></div>
      ...
      {mode === "task" ? (
        <form className="space-y-2" onSubmit={(event) => { event.preventDefault(); void saveTask(); }}>
          <Input autoFocus value={form.title} onChange={(event) => setForm((prev) => ({ ...prev, title: event.target.value }))} placeholder="Título de la tarea" className="h-9 rounded-xl border-[#dce8e2] bg-white text-sm" />
          <Textarea value={form.description} onChange={(event) => setForm((prev) => ({ ...prev, description: event.target.value }))} placeholder="Descripción opcional" rows={2} className="min-h-[58px] rounded-xl border-[#dce8e2] bg-white text-sm" />
          <div className="grid grid-cols-2 gap-2">
            <Input type="date" value={form.due_date} onChange={(event) => setForm((prev) => ({ ...prev, due_date: event.target.value }))} className="h-9 rounded-xl border-[#dce8e2] bg-white text-xs" />
            <Select value={form.priority} onValueChange={(value) => setForm((prev) => ({ ...prev, priority: value }))}>
              <SelectTrigger className="h-9 rounded-xl border-[#dce8e2] bg-white text-xs"><SelectValue /></SelectTrigger>
              <SelectContent>{PRIORITIES.map((priority) => <SelectItem key={priority} value={priority}>{priority}</SelectItem>)}</SelectContent>
            </Select>
          </div>
          <div className="flex gap-2 pt-1"><Button type="button" variant="outline" size="sm" className="h-9 flex-1 rounded-xl" onClick={() => setOpen(false)} disabled={saving}>Cerrar</Button><Button type="submit" size="sm" className="h-9 flex-1 rounded-xl bg-[#00a884] hover:bg-[#008f72]" disabled={saving || !form.title.trim()}><Save className="mr-1.5 h-3.5 w-3.5" /> {saving ? "..." : "Guardar"}</Button></div>
        </form>
      ) : (
        <form className="space-y-2" onSubmit={(event) => { event.preventDefault(); void saveNote(); }}>
          <Textarea autoFocus value={note} onChange={(event) => setNote(event.target.value)} placeholder="Escribe una nota para el equipo..." rows={4} className="min-h-[92px] rounded-xl border-[#dce8e2] bg-white text-sm" />
          <div className="flex gap-2 pt-1"><Button type="button" variant="outline" size="sm" className="h-9 flex-1 rounded-xl" onClick={() => setOpen(false)} disabled={saving}>Cerrar</Button><Button type="submit" size="sm" className="h-9 flex-1 rounded-xl bg-[#00a884] hover:bg-[#008f72]" disabled={saving || !note.trim()}><Save className="mr-1.5 h-3.5 w-3.5" /> {saving ? "..." : "Guardar nota"}</Button></div>
        </form>
      )}
    </section>,
    target,
