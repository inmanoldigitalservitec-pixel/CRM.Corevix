...
begin
  if exists (
    ...
    where conrelid = 'public.drive_files'::regclass
      and conname = 'drive_files_linked_type_check'
  ) then
    alter table public.drive_files drop constraint drive_files_linked_type_check;
  end if;
  ...
  if not exists (
    ...
    alter table public.drive_files
      add constraint drive_files_linked_type_check
      check (linked_type = any (array['project','task','client','lead','proposal','invoice']));
  end if;
end $$;

create index if not exists idx_drive_files_company_lead
