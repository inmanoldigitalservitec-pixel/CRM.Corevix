-- Fix: allow create_project_from_paid_invoice to run even when auth.uid() is null
-- (e.g. when testing from SQL Editor), without widening execute permissions.
--
-- Rules:
-- - Keep signature: public.create_project_from_paid_invoice(p_invoice_id uuid)
-- - Do not raise when auth.uid() is null; use it only for created_by when available.
-- - Still GRANT execute only to authenticated.
-- - Resolve product_id from invoice.product_id OR proposal.product_id.
-- - If no product_id or no active workflow, create project anyway and skip tasks.
-- - Idempotent: reuse project by invoice_id or proposal_id.
...
begin
  -- Resolve company scope when available (normal app usage).
  -- When auth.uid() is null (SQL Editor), fallback to invoice.company_id after loading by id.
  ...
  -- Resolve product_id from invoice or proposal
  ...
  -- If we can't resolve a product, create the project but skip tasks.
