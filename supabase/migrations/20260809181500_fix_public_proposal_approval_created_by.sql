-- Keep public proposal approval tolerant of legacy proposals whose created_by
-- no longer points to a valid public.profiles row.
...
begin
  ...
  select pr.id
    into v_invoice_created_by
  from public.profiles pr
  where pr.id = v_proposal.created_by
  limit 1;
  ...
  if v_created_invoice then
    ...
    perform public.log_activity_event(
      ...
      'invoice_created',
      'invoices',
      coalesce(v_invoice.created_by, v_invoice_created_by),
      v_invoice.id,
