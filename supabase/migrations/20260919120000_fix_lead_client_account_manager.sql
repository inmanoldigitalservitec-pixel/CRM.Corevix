-- Fix lead-to-client conversion: translate leads.assigned_to (auth.users.id)
-- to clients.account_manager (profiles.id) before inserting/updating clients.
...
begin
  ...
  -- leads.assigned_to stores auth.users.id, while clients.account_manager
  -- references profiles.id. Resolve the relationship before writing clients.
  if v_lead.assigned_to is not null then
    ...
    where p.company_id = v_lead.company_id
      and (
        p.user_id = v_lead.assigned_to
        or p.id = v_lead.assigned_to
      )
