alter table public.contracts
  ...
  add column if not exists contract_value_base numeric(14, 2);

create or replace function public.contracts_currency_sync_trigger()
returns trigger
language plpgsql
security definer
...
begin
  ...
  new.exchange_rate := coalesce(
    nullif(new.exchange_rate, 0),
    case when v_currency = new.base_currency then 1 else v_settings.usd_to_dop_rate end
  );
  ...
  new.contract_value_base := public.crm_convert_currency_amount(
...
drop trigger if exists contracts_currency_sync on public.contracts;
create trigger contracts_currency_sync
before insert or update on public.contracts
for each row execute function public.contracts_currency_sync_trigger();
