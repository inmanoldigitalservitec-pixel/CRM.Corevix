-- Fix project creation after final invoice payment.
-- projects.created_by references public.profiles(id), not auth.users(id).
-- Keep auth.uid() for session lookup but persist the profile id in both projects and tasks.
