-- Fix create_project_from_paid_invoice: remove enum cast to public.task_priority
-- because current DB uses text priority values in public.tasks.
...
begin
  ...
  select
    ...
    'To Do',
    case
      when lower(coalesce(nullif(s.default_priority, ''), '')) in ('low', 'medium', 'high', 'urgent')
        then initcap(lower(coalesce(nullif(s.default_priority, ''), 'Medium')))
      else
        'Medium'
    end,
    null,
