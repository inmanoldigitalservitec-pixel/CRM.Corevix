-- Fix create_project_from_paid_invoice due_date calculation:
-- cast cumulative workflow days to int to avoid date + bigint errors.
...
begin
  ...
  select
    ...
    s.description,
    'To Do',
    coalesce(nullif(s.default_priority, ''), 'Medium'),
    null,
    (current_date + ((sum(coalesce(s.default_duration_days, 1)) over (
