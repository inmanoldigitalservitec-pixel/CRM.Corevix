-- Ensure one project per invoice and make project creation idempotent under concurrency.

do $$
declare
  v_duplicate_count integer := 0;
  v_has_equivalent_unique_index boolean := false;
begin
  select count(*)
  ...
  if v_duplicate_count > 0 then
    raise exception 'Cannot create projects(company_id, invoice_id) unique index: duplicate invoice projects exist';
  end if;
  ...
  select exists (
    ...
    where nsp.nspname = 'public'
      ...
      and idx.indisunique
      and pg_get_indexdef(cls.oid) ilike '%(company_id, invoice_id)%'
      and coalesce(pg_get_expr(idx.indpred, idx.indrelid), '') ilike '%invoice_id IS NOT NULL%'
  )
  into v_has_equivalent_unique_index;

  if not v_has_equivalent_unique_index then
    create unique index projects_company_invoice_unique_idx
      on public.projects(company_id, invoice_id)
      where invoice_id is not null;
  end if;
end;
$$;
...
begin
  ...
  -- Exact idempotency: invoice_id is the canonical relation.
  select pr.*
    into v_project
  ...
  -- Compatibility fallback: older flows may have linked only proposal_id.
  if v_invoice.proposal_id is not null then
    select pr.*
      into v_project
    from public.projects pr
    where pr.company_id = v_company_id
      and pr.proposal_id = v_invoice.proposal_id
    order by pr.created_at desc
    limit 1;
  ...
  if v_client_label is null and v_proposal.id is not null then
    v_client_label := nullif(btrim(coalesce(v_proposal.title, '')), '');
  end if;
  ...
  -- Tasks are created only after a new project insert succeeds.
