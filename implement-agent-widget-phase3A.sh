...
echo "=========================================="
echo " Implementing Agent Widget Phase 3A"
echo " Adapter + Connected Component Only"
echo "=========================================="
echo ""

COMPONENT_DIR="./src/components/agent"
ADAPTER_FILE="$COMPONENT_DIR/agentPromptPayloadAdapter.ts"
CONNECTED_FILE="$COMPONENT_DIR/AgentCommandWidgetConnected.tsx"
INDEX_FILE="$COMPONENT_DIR/index.ts"
BACKUP_DIR="./agent-ready-patch/backups-agent-phase3A-$(date +%Y%m%d-%H%M%S)"
...
for file in "$ADAPTER_FILE" "$CONNECTED_FILE" "$INDEX_FILE"; do
  if [ -f "$file" ]; then
...
function normalizeRecoveryPlan(plan: AgentPromptRecoveryPlan, payload: AgentPromptPayload): AgentRecoveryPlan {
  ...
  const suggestedActions =
    plan.suggested_actions && plan.suggested_actions.length > 0
      ? plan.suggested_actions.map((action) => ({
          type: cleanText(action.type, "review_record"),
          label: cleanText(action.label, "Revisar caso"),
          requires_confirmation: actionRequiresConfirmation(action),
        }))
  ...
  return {
    case_key: caseKey,
    title: cleanText(plan.plan_title || plan.title, "Plan de sanación"),
    severity: normalizeSeverity(plan.priority || plan.severity),
    message: cleanText(plan.message, "El agente preparó un plan de sanación para este caso."),
...
echo "Writing connected component..."

cat > "$CONNECTED_FILE" <<'TSX'
...
echo "grep -RIn \"AgentCommandWidgetConnected\\|agentPromptPayloadToRecoveryPlans\" ./src/components/agent"
echo "npm run build"
