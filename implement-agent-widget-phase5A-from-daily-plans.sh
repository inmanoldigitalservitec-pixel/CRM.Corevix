...
BACKUP_DIR="./agent-ready-patch/backups-phase5A-widget-writer-from-daily-plans-$(date +%Y%m%d-%H%M%S)"

mkdir -p "$BACKUP_DIR"
mkdir -p "./apps/agent-worker/src"

if [ -f "$FILE" ]; then
  cp "$FILE" "$BACKUP_DIR/agent-widget-contract.ts.bak"
fi
...
echo ""
echo "Validation:"
grep -n "buildAgentWidgetContractFromDailyPlans\|writeAgentWidgetContractFromDailyPlans\|agent_widget_contracts\|agent_widget_contract_v1" "$FILE"
