...
function normalizeRequiredFieldName(field: string, action?: AnyRecord) {
	...
	const actionText = [action?.type, action?.label, action?.tool_hint, action?.reason]
	...
	if (normalized === 'time slot' || normalized === 'time_slot' || normalized.includes('horario') || normalized.includes('hora')) {
		return 'time_slot';
	}

	if (normalized.includes('fecha') || normalized.includes('due date') || normalized.includes('delivery date')) {
		if (
...
function humanToolLabel(toolHint: unknown, action?: AnyRecord) {
	...
	const actionText = [action?.type, action?.label, action?.reason].map((item) => cleanText(item).toLowerCase()).join(' ');
...
function inferValueFromInstruction(field: string, userInstruction: string) {
	...
	if (normalized === 'new_due_date' || normalized === 'due_at' || normalized === 'dueat' || normalized === 'date') {
		return inferDateFromInstruction(userInstruction);
	}

	if (normalized === 'contact_phone' || normalized === 'contactphone' || normalized === 'phone' || normalized === 'telephone') {
		return inferPhoneFromInstruction(userInstruction);
	}

	if (normalized === 'time_slot' || normalized === 'timeslot' || normalized === 'appointment_time' || normalized === 'time') {
...
function inferRequiredFields(action: AnyRecord, userInstruction: string) {
	const existing = Array.isArray(action?.required_fields)
		? action.required_fields.map((field: unknown) => normalizeRequiredFieldName(cleanText(field), action)).filter(Boolean)
		: [];
	...
	if (
		(type === 'move_due_date' || tool === 'reschedule_task' || text.includes('mover') || text.includes('reprogram')) &&
		!action?.payload?.new_due_date
	) {
...
export function resolveAgentActionIntent(input: AgentResolveIntentRequest): AgentResolvedActionIntent {
	...
	const actionLabel = cleanText(userInstruction || selectedAction.label || selectedAction.type, 'Acción de Autopilot');
	...
	return {
		...
		intent: {
			...
			label: actionLabel,
			reason: userInstruction ? `Instrucción escrita por el usuario: ${userInstruction}` : selectedAction.reason || null,
