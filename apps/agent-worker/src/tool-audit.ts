...
function pickTarget(args: Record<string, any>, result: ToolResult) {
	...
	const fullLeadName = data.first_name || data.last_name ? `${data.first_name ?? ''} ${data.last_name ?? ''}`.trim() : null;
...
export async function auditAgentToolAction(ctx: ToolContext, call: ToolCall, result: ToolResult): Promise<ToolResult> {
