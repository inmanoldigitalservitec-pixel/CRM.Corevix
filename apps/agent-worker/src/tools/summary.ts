...
export async function crmSummaryTool(ctx: ToolContext): Promise<ToolResult> {
	...
	const [leadsResult, tasksResult, invoicesResult, projectsResult] = await Promise.all([
		ctx.supabase.from('leads').select('id,status,created_at').eq('company_id', ctx.companyId).gte('created_at', start.toISOString()),

		ctx.supabase.from('tasks').select('id,status,due_date').eq('company_id', ctx.companyId).neq('status', 'completed'),

		ctx.supabase.from('invoices').select('id,status,total,due_date').eq('company_id', ctx.companyId).neq('status', 'paid'),

		ctx.supabase.from('projects').select('id,status').eq('company_id', ctx.companyId).neq('status', 'completed'),
	]);
