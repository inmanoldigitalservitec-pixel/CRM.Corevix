...
const EVENT_SELECT =
  "id,title,description,location,type,status,start_at,end_at,all_day,related_task_id,created_at,updated_at";
...
async function findTask(ctx: ToolContext, args: any): Promise<ToolResult & { task?: any }> {
  ...
  if (matches.length > 1 && !taskId) {
    ...
    const exact = matches.find((item: any) => String(item.title || "").toLowerCase().trim() === normalizedQuery);
    if (!exact) {
      return {
        ok: false,
...
export async function updateTaskTool(ctx: ToolContext, args: any): Promise<ToolResult> {
  ...
  for (const key of ["title", "description", "due_date", "assigned_to", "related_lead_id", "related_client_id", "related_deal_id", "related_project_id"]) {
