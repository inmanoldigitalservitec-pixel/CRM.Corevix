...
const LEAD_STATUSES = ['New', 'Contacted', 'Qualified', 'Proposal Needed', 'Proposal Sent', 'Negotiation', 'Won', 'Lost', 'Not Interested'];
...
async function findLead(ctx: ToolContext, args: any): Promise<ToolResult & { lead?: any }> {
	...
	if (matches.length > 1 && !leadId) {
		...
		const exact = matches.find((item: any) => `${item.first_name ?? ''} ${item.last_name ?? ''}`.toLowerCase().trim() === normalizedQuery);
