...
export async function resolveCrmAssignee(
	ctx: ToolContext,
	args: any,
): Promise<ToolResult & { assigneeId?: string; profileId?: string; profile?: any }> {
	...
	if (matches.length === 1)
		return { ok: true, assigneeId: matches[0].user_id || matches[0].id, profileId: matches[0].id, profile: matches[0] };
	...
	if (matches.length > 1) {
		return {
			...
			data: matches
				.slice(0, 8)
				.map((profile: any) => ({ id: profile.id, full_name: profile.full_name, email: profile.email, is_active: profile.is_active })),
		};
	}
...
export function hasAssigneeInput(args: any) {
	return ['assigned_to', 'assignee', 'owner', 'owner_id', 'user', 'user_id', 'profile_id', 'responsible', 'responsable', 'assign_to'].some(
